			...
			public void actionPerformed(ActionEvent e) {
				try {
					if (textNazwa.getText().length() > 0 && ((Integer) spinnerKalorycznosc.getValue()) > 0) {
						...
						db.addIngredient(textNazwa.getText(), checkboxLak.isSelected(), checkboxGlut.isSelected(),
								(Integer) spinnerKalorycznosc.getValue(), checkboxMies.isSelected());
						if (sessionDB.getTransaction().isActive()) {
							sessionDB.getTransaction().commit();
						}
						JOptionPane.showMessageDialog(null, "Składnik dodany pomyślnie", "Informacja",
						...
						JOptionPane.showMessageDialog(null, "Uzupełnij dane składnika", "Błąd",
								JOptionPane.INFORMATION_MESSAGE);
					}
				} catch (Exception e2) {
